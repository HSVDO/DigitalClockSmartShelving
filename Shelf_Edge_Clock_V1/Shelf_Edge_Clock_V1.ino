...
#define LEDCLOCK_COUNT      84
#define LEDDOWNLIGHT_COUNT  16
#define LEDCLOCKDOT_COUNT   4
...
int ledsPerSegment = LEDCLOCK_COUNT / 4 / 7;
...
void setup() {
  ...
  pinMode(LEDCLOCK_PIN, OUTPUT);
  pinMode(LEDCLOCKDOT_PIN, OUTPUT);
  //Clock.begin();
  ...
  stripClock.setBrightness(128); // Set inital BRIGHTNESS (max = 255)
...
void initClock(){
  MyDateAndTime.Day    = 27;
  MyDateAndTime.Month  = 11;
  MyDateAndTime.Year   = 21; 
  MyDateAndTime.Hour   = 18;
  MyDateAndTime.Minute = 56;
  MyDateAndTime.Second = 30;
  // Then write it to the clock
  //Clock.write(MyDateAndTime);
}

void increaseClock(){
  int hour = MyDateAndTime.Hour;
  int minute = MyDateAndTime.Minute;
...
void displayTheTime(){
  ...
  displayNumber(secondMinuteDigit, ledsPerNumber, clockMinuteColour);  
 
  int firstHourDigit = MyDateAndTime.Hour % 10;
  displayNumber(firstHourDigit, 2*ledsPerNumber, clockHourColour);

  int secondHourDigit = floor(MyDateAndTime.Hour / 10); //work out the value for the fourth digit and then display it
  displayNumber(secondHourDigit, 3*ledsPerNumber, clockHourColour);

  Serial.println("Current time:");
  Serial.print(secondHourDigit);  Serial.print(firstHourDigit); Serial.print(":"); Serial.print(secondMinuteDigit); Serial.print(firstMinuteDigit);  
  Serial.println("");
  stripClock.show();
}
